...
export const FIELDS: Partial<Record<ForgeModelSlug, Fields>> = {
  ...
  "phone-stand": {
    base_w:      { label: "Ancho base (mm)",      type: "number", step: 1,   min: 55, defaultValue: 90 },
    base_d:      { label: "Fondo base (mm)",      type: "number", step: 1,   min: 65, defaultValue: 110 },
    angle_deg:   { label: "Ángulo respaldo (°)",  type: "number", step: 1,   min: 45, max: 80, defaultValue: 62 },
    slot_w:      { label: "Holgura teléfono (mm)",type: "number", step: 0.5, min: 7,  defaultValue: 12 },
    slot_d:      { label: "Fondo apoyo (mm)",     type: "number", step: 0.5, min: 8,  defaultValue: 16 },
    usb_clear_h: { label: "Canal USB-C (mm)",     type: "number", step: 0.5, min: 3,  defaultValue: 7 },
    back_h:      { label: "Altura respaldo (mm)", type: "number", step: 1,   min: 70, defaultValue: 105 },
    wall:        { label: "Grosor (mm)",          type: "number", step: 0.5, min: 2.4, defaultValue: 4 },
  },
  "tablet-stand": {
  ...
  "raspi-case": {
    board_w:     { label: "Largo PCB Pi 4 (mm)",      type: "number", step: 0.5, min: 80, defaultValue: 85.0 },
    board_l:     { label: "Ancho PCB Pi 4 (mm)",      type: "number", step: 0.5, min: 50, defaultValue: 56.0 },
    board_h:     { label: "Altura componentes (mm)",  type: "number", step: 0.5, min: 12, defaultValue: 17.0 },
    port_clear:  { label: "Holgura conectores (mm)",  type: "number", step: 0.25, min: 0.5, defaultValue: 1.0 },
    wall:        { label: "Grosor pared (mm)",        type: "number", step: 0.2, min: 1.8, defaultValue: 2.2 },
    bottom:      { label: "Grosor fondo (mm)",        type: "number", step: 0.2, min: 1.8, defaultValue: 2.4 },
    lid_t:       { label: "Grosor tapa (mm)",         type: "number", step: 0.2, min: 1.8, defaultValue: 2.2 },
    standoff_h:  { label: "Altura separadores (mm)",  type: "number", step: 0.5, min: 2.5, defaultValue: 4.0 },
  },
  "go-pro-mount": {
  ...
  "wall-bracket": {
    length_mm:    { label: "Largo ala (mm)",        type: "number", step: 1,   min: 50, defaultValue: 120 },
    width_mm:     { label: "Ancho (mm)",            type: "number", step: 1,   min: 20, defaultValue: 40 },
    height_mm:    { label: "Altura ala (mm)",       type: "number", step: 1,   min: 40, defaultValue: 80 },
    thickness_mm: { label: "Grosor (mm)",           type: "number", step: 0.5, min: 2.5, defaultValue: 4 },
  },
  "vesa-shelf": {
    vesa:         { label: "Patrón VESA (mm)",      type: "number", step: 25,  min: 75,  max: 200, defaultValue: 100 },
    thickness:    { label: "Grosor (mm)",           type: "number", step: 0.5, min: 2.5, defaultValue: 4 },
    shelf_width:  { label: "Ancho estante (mm)",    type: "number", step: 5,   min: 100, defaultValue: 180 },
    shelf_depth:  { label: "Fondo estante (mm)",    type: "number", step: 5,   min: 60,  defaultValue: 120 },
    lip_height:   { label: "Altura pestaña (mm)",   type: "number", step: 1,   min: 0,   defaultValue: 15 },
    rib_count:    { label: "Nº refuerzos",          type: "number", step: 1,   min: 0,   max: 8, defaultValue: 3 },
    hole_d:       { label: "Ø taladro VESA (mm)",   type: "number", step: 0.1, min: 3,   defaultValue: 5 },
  },
  "enclosure-ip65": {
    length:        { label: "Largo exterior (mm)",   type: "number", step: 1,   min: 50, defaultValue: 120 },
    width:         { label: "Ancho exterior (mm)",   type: "number", step: 1,   min: 35, defaultValue: 68 },
    height:        { label: "Altura exterior (mm)",  type: "number", step: 1,   min: 20, defaultValue: 45 },
    wall:          { label: "Grosor pared (mm)",     type: "number", step: 0.2, min: 2,  defaultValue: 3 },
    lid_thickness: { label: "Grosor tapa (mm)",      type: "number", step: 0.2, min: 2,  defaultValue: 3 },
    lid_gap:       { label: "Separación tapa (mm)",  type: "number", step: 0.5, min: 1,  defaultValue: 2 },
  },
  "qr-plate": {
    length:      { label: "Largo placa (mm)",        type: "number", step: 1,   min: 40, defaultValue: 90 },
    width:       { label: "Ancho placa (mm)",        type: "number", step: 1,   min: 20, defaultValue: 38 },
    thickness:   { label: "Grosor (mm)",             type: "number", step: 0.5, min: 2,  defaultValue: 8 },
    slot_mm:     { label: "Separación agujeros (mm)",type: "number", step: 1,   min: 10, defaultValue: 22 },
    screw_d_mm:  { label: "Ø tornillo (mm)",         type: "number", step: 0.1, min: 3,  defaultValue: 6.5 },
  },
  "monitor-stand": {
