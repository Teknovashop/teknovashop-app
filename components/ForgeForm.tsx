...
type OpCutout = OpBase & {
  ...
  d_mm?: number; w_mm?: number; h_mm?: number;
  depth_mm?: number;
};
type OpText = OpBase & {
  type: "text";
  text: string; x_mm: number; y_mm: number;
  size_mm: number; depth_mm: number; engrave?: boolean;
};
...
type OpArray = OpBase & {
  ...
  nx: number; ny: number; dx_mm: number; dy_mm: number;
  d_mm?: number; w_mm?: number; h_mm?: number;
};
...
const emit  = <T,>(name: string, detail?: T) => { try { window.dispatchEvent(new CustomEvent(name, { detail })); } catch {} };
...
export default function ForgeForm({
  ...
  const modelOptions = useMemo(
    () => MODELS.map((m) => ({ value: toSnake(m.slug), label: m.name })),
    []
  );
  ...
  // Sincronización con visor
  useEffect(() => { emit("forge:set-model", { model }); emit("forge:refresh", { reason: "model-change" }); }, [model]);
  ...
  // 👉 Escuchar ALT+click del visor para añadir agujeros
  useEffect(() => {
    const onAddHole = (ev: Event) => {
      const h = (ev as CustomEvent).detail as Hole | undefined;
      if (!h) return;
      setHoles((prev) => [...prev, h]);
    };
  ...
  const handleGenerate = async () => {
    ...
    try {
      const payload: any = { model, params, holes, operations };
      ...
      if (url) {
        onGenerated?.(url);
        // alineado con el visor (por si quieres por eventos)
    ...
    <div className="rounded-2xl border border-neutral-200 bg-white p-4 shadow-sm">
      ...
      <div className="grid grid-cols-2 gap-3">
        <label className="col-span-2 text-sm">
          ...
          <select className="w-full rounded-md border border-neutral-300 bg-white px-3 py-2"
      ...
      <div className="mt-4">
        <div className="mb-2 flex items-center justify-between">
          ...
          <button type="button" className="rounded-md border border-neutral-300 px-2 py-1 text-sm"
            ...
            <div key={i} className="grid grid-cols-4 items-end gap-2">
              <label className="text-xs">
                ...
                <input type="number" className="w-full rounded-md border border-neutral-300 px-2 py-1.5 text-sm"
                       value={h.x_mm}
                       onChange={(e) => setHoles((prev) => prev.map((hh, idx) => (idx === i ? { ...hh, x_mm: n(e.target.value, hh.x_mm) } : hh)))} />
              </label>
              <label className="text-xs">
                <span className="mb-0.5 block text-neutral-600">Y (mm)</span>
                <input type="number" className="w-full rounded-md border border-neutral-300 px-2 py-1.5 text-sm"
                       value={h.y_mm}
                       onChange={(e) => setHoles((prev) => prev.map((hh, idx) => (idx === i ? { ...hh, y_mm: n(e.target.value, hh.y_mm) } : hh)))} />
              </label>
              <label className="text-xs">
                <span className="mb-0.5 block text-neutral-600">Ø (mm)</span>
                <input type="number" className="w-full rounded-md border border-neutral-300 px-2 py-1.5 text-sm"
                       value={h.d_mm}
                       onChange={(e) => setHoles((prev) => prev.map((hh, idx) => (idx === i ? { ...hh, d_mm: n(e.target.value, hh.d_mm) } : hh)))} />
              </label>
              <button type="button" className="rounded-md border border-neutral-300 px-2 py-1 text-xs"
        ...
        <div className="grid gap-3">
          {operations.map((op) => (
            <div key={op.id} className="rounded-md border p-3">
              <div className="mb-2 flex items-center justify-between">
                <select className="rounded-md border bg-white px-2 py-1 text-xs" value={op.type}
                        onChange={(e) => { const t = e.target.value as Operation["type"]; removeOp(op.id); setTimeout(() => addOperation(t), 0); }}>
              ...
              {op.type === "round" && (
                <label className="text-xs">
                  ...
                  <input type="number" className="w-full rounded-md border px-2 py-1 text-sm"
                         ...
                         onChange={(e) => patchOp(op.id, "r_mm", n(e.target.value, (op as OpRound).r_mm))} min={0} step={0.5} />
                </label>
              )}
              ...
              {op.type === "text" && (
                <div className="grid grid-cols-4 gap-2">
                  <label className="col-span-4 text-xs">
                    ...
                    <input type="text" className="w-full rounded-md border px-2 py-1 text-sm"
                           value={(op as OpText).text} onChange={(e) => patchOp(op.id, "text", e.target.value)} />
                  </label>
                  <label className="text-xs">
                    <span className="mb-0.5 block text-neutral-600">X (mm)</span>
                    <input type="number" className="w-full rounded-md border px-2 py-1 text-sm"
                  ...
                  <label className="col-span-4 mt-1 inline-flex items-center gap-2 text-xs">
                    <input type="checkbox" checked={(op as OpText).engrave ?? true}
              ...
              {op.type === "cutout" && (
                <div className="grid grid-cols-6 gap-2">
                  <label className="text-xs col-span-2">
                    ...
                    <select className="w-full rounded-md border bg-white px-2 py-1.5 text-sm"
                            value={(op as OpCutout).shape} onChange={(e) => patchOp(op.id, "shape", e.target.value)}>
              ...
              {op.type === "array" && (
                <div className="grid grid-cols-8 gap-2">
                  <label className="text-xs col-span-2">
                    <span className="mb-0.5 block text-neutral-600">Shape</span>
                    <select className="w-full rounded-md border bg-white px-2 py-1.5 text-sm"
                            value={(op as OpArray).shape} onChange={(e) => patchOp(op.id, "shape", e.target.value)}>
                      <option value="circle">circle</option>
                      <option value="rect">rect</option>
                    </select>
                  </label>
      ...
      <div className="mt-4 flex items-center gap-3">
        <button type="button" className="rounded-md bg-black px-4 py-2 text-white"
                onClick={handleGenerate} disabled={busy || !API_BASE}>
          {busy ? "Generando…" : "Generar STL"}
        </button>
        {!API_BASE && (
          <span className="text-xs text-neutral-500">
            Configura <code>NEXT_PUBLIC_FORGE_API_URL</code> o <code>NEXT_PUBLIC_BACKEND_URL</code>.
          </span>
        )}
      </div>
      ...
      <div className="mt-2 text-sm text-neutral-700">
        ...
        {svgUrl && (
          <div className="mt-2">
            <a href={svgUrl} target="_blank" rel="noreferrer" className="underline">Descargar SVG</a>
          </div>
        )}
      </div>
