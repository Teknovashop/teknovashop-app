...
export default function ForgeForm({ onGenerated }: ForgeFormProps) {
  ...
  // Normalizador (sin replaceAll -> compatible con TS config más estrictas)
  const normalizedModel = useMemo(() => model.replace(/-/g, "_"), [model]);
  ...
  async function handleGenerate() {
    ...
    try {
      const res = await fetch(`${base}/generate`, {
        ...
        body: JSON.stringify({
          model: normalizedModel, // <- usamos el nombre normalizado
          params: {
            length_mm: length,
            width_mm: width,
            height_mm: height,
          ...
          holes: [], // por ahora vacío (UI de ediciones la añadimos después)
        }),
      });
